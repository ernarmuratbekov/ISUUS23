...
class Project3:
    ...
    def plot_bar_chart(self, df, category_column, value_column):
        ...
        if df[category_column].dtype in ['object', 'category'] or df[category_column].dtype.name == 'category':
            if df[value_column].dtype in ['int64', 'float64']:
                ...
                ax.set_title(f'Mean of {value_column} by {category_column}')
                ax.set_xlabel(category_column)
                ax.set_ylabel(f'Mean of {value_column}')
                st.pyplot(fig)

                # Display the mean values for each category
                st.write(f'Mean of {value_column} by {category_column}:')
    ...
    def app(self):
        ...
        if upload is not None:
            ...
            # Select colmns for categorical and numeric data
